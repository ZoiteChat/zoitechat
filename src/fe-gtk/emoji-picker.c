...
static void emoji_search_stop_cb (GtkSearchEntry *entry, gpointer userdata);
 ...
 * font without changing the fonts of surrounding widgets. The private
 * font map resolves ZoiteChat Emoji to the same bundled file everywhere. */
static void
emoji_picker_ensure_css (void)
{
	...
	gtk_css_provider_load_from_data (provider,
		".zc-emoji-cell {"
		"  font-family: \"ZoiteChat Emoji\", \"Noto Color Emoji\","
...
/* GtkEntry can select all text when focused; preserve its selection and
 * editability instead of temporarily making a read-only entry writable. */
static void
emoji_picker_focus_target (EmojiPicker *picker)
{
	...
	if (!picker->target)
		return;
	/* Closing a window also closes its popovers. Moving focus during that
	 * teardown can make GtkWindow reference an already destroyed widget. */
	if (gtk_widget_in_destruction (picker->target) ||
		gtk_widget_in_destruction (gtk_widget_get_toplevel (picker->target)))
		return;
	...
	if (selected)
		gtk_editable_select_region (editable, pos == start ? end : start, pos);
	else
...
{
	GtkEditable *editable;
	int pos, start, end, max_length, length;
	...
	/* GtkEntry otherwise truncates at a code point, possibly leaving half
	 * a flag, a dangling joiner or a skin-tone modifier. Reject as a unit. */
	max_length = gtk_entry_get_max_length (GTK_ENTRY (editable));
	length = gtk_entry_get_text_length (GTK_ENTRY (editable));
	if (gtk_editable_get_selection_bounds (editable, &start, &end))
		length -= end - start;
	if (length + g_utf8_strlen (sequence, -1) > (max_length ? max_length : 65535))
	{
		gtk_widget_error_bell (picker->target);
		return;
	}
	...
	emoji_picker_focus_target (picker);
}
...
emoji_search_activate_cb (GtkEntry *entry, gpointer userdata)
{
	...
	/* search-changed is delayed by GTK; Enter must use the current query. */
	emoji_picker_refresh (picker);
...
emoji_picker_build_contents (EmojiPicker *picker)
{
	...
	g_signal_connect (G_OBJECT (picker->search_entry), "stop-search",
		G_CALLBACK (emoji_search_stop_cb), picker);
...
static void
emoji_search_stop_cb (GtkSearchEntry *entry, gpointer userdata)
{
	EmojiPicker *picker = userdata;
...
static void
emoji_entry_insert_cb (GtkEntry *entry, GtkWidget *button)
{
	/* Covers GtkEntry's context menu and Ctrl+period/Ctrl+semicolon. */
...
emoji_picker_button_new (GtkWidget *target_entry)
{
	...
	emoji_font_apply (picker->button);
	gtk_button_set_relief (GTK_BUTTON (picker->button), GTK_RELIEF_NONE);
	gtk_widget_set_can_focus (picker->button, TRUE);
	gtk_widget_set_focus_on_click (picker->button, FALSE);
	...
	emoji_font_apply (picker->popover);
	g_object_set_data_full (G_OBJECT (picker->popover), "emoji-picker",
		picker, emoji_picker_free);
	/* GtkEntry added this action in 3.22.27; earlier supported GTK 3.22
	 * runtimes still use the button without emitting an unknown signal. */
	if (g_signal_lookup ("insert-emoji", GTK_TYPE_ENTRY))
		g_signal_connect_object (target_entry, "insert-emoji",
			G_CALLBACK (emoji_entry_insert_cb), picker->button, 0);
