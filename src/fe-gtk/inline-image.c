...
image_fetch_drop_connection (image_fetch *fetch)
{
	...
	if (fetch->ssl_source)
	{
		g_source_destroy (fetch->ssl_source);
		g_source_unref (fetch->ssl_source);
		fetch->ssl_source = NULL;
	}
	if (fetch->ssl)
	{
		_SSL_close (fetch->ssl);
		fetch->ssl = NULL;
	}
	if (fetch->ssl_ctx)
	{
		_SSL_context_free (fetch->ssl_ctx);
		fetch->ssl_ctx = NULL;
	}
#endif
	if (fetch->conn)
...
static gboolean
image_fetch_read_result (image_fetch *fetch, gssize n)
{
	if (n == 0)
	{
...
image_fetch_read_done (GObject *source, GAsyncResult *res, gpointer user_data)
{
	...
	if (image_fetch_read_result (fetch, n))
...
static void
image_fetch_plain_read_done (GObject *source, GAsyncResult *res, gpointer user_data)
{
	image_fetch *fetch = user_data;
	GError *err = NULL;
...
static void
image_fetch_plain_write_done (GObject *source, GAsyncResult *res, gpointer user_data)
{
	image_fetch *fetch = user_data;
	GError *err = NULL;
	gssize n = g_output_stream_write_finish (G_OUTPUT_STREAM (source), res, &err);
...
static void
image_fetch_ssl_error (image_fetch *fetch)
{
	char message[256];
	...
	if (code)
		...
		g_strlcpy (message, _("TLS connection failed"), sizeof (message));
	image_fetch_fail (fetch, message);
}

static void
image_fetch_ssl_wait (image_fetch *fetch, GIOCondition condition,
						GSourceFunc callback)
{
	GSocket *socket = g_socket_connection_get_socket (fetch->conn);

	fetch->ssl_source = g_socket_create_source (socket, condition, NULL);
	g_source_set_callback (fetch->ssl_source, callback, fetch, NULL);
	g_source_attach (fetch->ssl_source, NULL);
}

static void
image_fetch_ssl_ready (image_fetch *fetch)
{
	g_source_unref (fetch->ssl_source);
	fetch->ssl_source = NULL;
}
...
static gboolean
image_fetch_read_ready (GSocket *socket, GIOCondition condition,
						 gpointer user_data)
{
	image_fetch *fetch = user_data;
	int error, n;
	...
	if (fetch->ssl_source)
		image_fetch_ssl_ready (fetch);
	n = SSL_read (fetch->ssl, fetch->readbuf, sizeof (fetch->readbuf));
	if (n > 0)
	{
		if (image_fetch_read_result (fetch, n))
			image_fetch_read_more (fetch);
	...
	error = SSL_get_error (fetch->ssl, n);
	if (error == SSL_ERROR_WANT_READ || error == SSL_ERROR_WANT_WRITE)
		image_fetch_ssl_wait (fetch, error == SSL_ERROR_WANT_READ ? G_IO_IN : G_IO_OUT,
								 (GSourceFunc) image_fetch_read_ready);
	else if (error == SSL_ERROR_ZERO_RETURN)
		image_fetch_complete (fetch);
	else
		image_fetch_ssl_error (fetch);
...
{
	if (fetch->ssl)
		image_fetch_read_ready (NULL, 0, fetch);
	else
	{
		GInputStream *in = g_io_stream_get_input_stream (G_IO_STREAM (fetch->conn));
		...
		g_input_stream_read_async (in, fetch->readbuf, sizeof (fetch->readbuf),
								 G_PRIORITY_DEFAULT, NULL,
								 image_fetch_plain_read_done, fetch);
	}
}

static gboolean
image_fetch_write_ready (GSocket *socket, GIOCondition condition,
						  gpointer user_data)
{
	image_fetch *fetch = user_data;
	int error, n;
	...
	if (fetch->ssl_source)
		image_fetch_ssl_ready (fetch);
	n = SSL_write (fetch->ssl, fetch->request + fetch->request_sent,
					 strlen (fetch->request) - fetch->request_sent);
	if (n > 0)
	{
		fetch->request_sent += n;
	...
	if (error == SSL_ERROR_WANT_READ || error == SSL_ERROR_WANT_WRITE)
		image_fetch_ssl_wait (fetch, error == SSL_ERROR_WANT_READ ? G_IO_IN : G_IO_OUT,
								 (GSourceFunc) image_fetch_write_ready);
	else
		image_fetch_ssl_error (fetch);
...
{
	if (fetch->ssl)
		image_fetch_write_ready (NULL, 0, fetch);
	else
	{
		GOutputStream *out = g_io_stream_get_output_stream (G_IO_STREAM (fetch->conn));
		...
		g_output_stream_write_async (out, fetch->request + fetch->request_sent,
									 strlen (fetch->request) - fetch->request_sent,
									 G_PRIORITY_DEFAULT, NULL,
									 image_fetch_plain_write_done, fetch);
	}
}

static gboolean
image_fetch_handshake_ready (GSocket *socket, GIOCondition condition,
							  gpointer user_data)
{
	image_fetch *fetch = user_data;
	X509 *cert;
	...
	if (fetch->ssl_source)
		image_fetch_ssl_ready (fetch);
	result = SSL_connect (fetch->ssl);
	if (result == 1)
	{
		cert = SSL_get_peer_certificate (fetch->ssl);
		if (SSL_get_verify_result (fetch->ssl) != X509_V_OK || cert == NULL ||
			 _SSL_check_hostname (cert, fetch->host) != 0)
		{
			if (cert)
				X509_free (cert);
			image_fetch_fail (fetch, _("TLS certificate verification failed"));
			return G_SOURCE_REMOVE;
		}
		X509_free (cert);
		image_fetch_write_more (fetch);
	...
	error = SSL_get_error (fetch->ssl, result);
	if (error == SSL_ERROR_WANT_READ || error == SSL_ERROR_WANT_WRITE)
		image_fetch_ssl_wait (fetch, error == SSL_ERROR_WANT_READ ? G_IO_IN : G_IO_OUT,
								 (GSourceFunc) image_fetch_handshake_ready);
	else
		image_fetch_ssl_error (fetch);
...
image_fetch_connected (GObject *source, GAsyncResult *res, gpointer user_data)
{
	...
	if (fetch->tls)
	{
		...
		fetch->ssl_ctx = _SSL_context_init (NULL);
		if (fetch->ssl_ctx == NULL || _SSL_set_verify (fetch->ssl_ctx, NULL) != NULL)
		{
			image_fetch_fail (fetch, _("TLS certificate store unavailable"));
			return;
		}
		fetch->ssl = _SSL_socket (fetch->ssl_ctx, g_socket_get_fd (socket));
		SSL_set_tlsext_host_name (fetch->ssl, fetch->host);
		image_fetch_handshake_ready (NULL, 0, fetch);
		return;
	}
#endif
	image_fetch_write_more (fetch);
